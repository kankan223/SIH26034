...
| 2026-09-09 20:00 | Perf optimization | Backend test-suite runtime reduced 63.5s→35.3s (~44% faster) by adding a session-scoped trained_classifier fixture (conftest.py) so the GradientBoosting model is trained once per suite instead of once per artifact/perf-warmup test; test_classification_under_50ms now measures inference only (<50ms target, was re-training + inference); artifact tests (TestModelArtifact) inspect the pre-trained pipeline via joblib.load without re-training; test_retrain_under_10s keeps its 3-run median but uses the fixture for warm-up; frontend unchanged (54/54 Vitest, 10.7s) | backend/tests/conftest.py, backend/tests/test_classification.py | VERIFIED | 546/546 PASS in 35.3s (was 63.5s); slowest: retrain_under_10s 9.97s, integration_with_cv_detection 4.37s; no regressions | Session fixture trades a little session-setup time for much less repeated work across the 7 classification tests that previously each called retrain_model() | AI Agent |
| 2026-09-10 02:30 | Perf optimization (OCR/CV fixtures) | Added session-scoped ocr_service_module + ocr_engine + cv_detection_module + cv_detection fixtures to conftest.py so PaddleOCR (~5s init) and cv_detection (~0.6s import + model warm) are paid once per suite; OCR perf tests now reuse the pre-initialized engine (test_extraction_under_3s 0.42s call, was loading PaddleOCR on first call); extraction integration test uses fixtures instead of inline imports; fixed trained_classifier fixture (was _get_ocr_engine → _load_ocr) and restored cv2 import in test_ocr_service.py helpers; 546/546 PASS in 37.3s; frontend 54/54 (10.0s) | backend/tests/conftest.py, backend/tests/test_ocr_service.py, backend/tests/test_extraction.py | VERIFIED | 546/546 PASS in 37.3s (35.3s baseline + ~2s OCR/CV fixture upfront load that was previously paid lazily by first-touch test); slowest: retrain_under_10s 10.12s, integration_with_cv_detection 3.97s setup + 0.47s call (PaddleOCR init moved to fixture, test body now fast); no regressions | OCR/CV fixtures make engine init deterministic at session setup rather than on first test call; net neutral on total time but more robust | AI Agent |
| 2026-09-09 19:30 | Real-photo retraining pipeline (Task 3.1.1 real-photo corpus) | Real-photo acquisition + auto-annotation pipeline built and evaluated: fetch_real_photos.py (open-licensed photos from Wikimedia Commons, append-safe, per-file licenses/attribution in CREDITS.md + manifest.json) + annotate_real_photos.py (YOLO-World zero-shot on concrete vocabulary box/carton/bottle/can/packet/pouch/jar/tube + label/sticker, cross-checked against fine-tuned detector; QC rejects blurry/monochrome/package-less frames; val pool restricted to strict-confidence or cross-model-agreed annotations) + build_combined_dataset.py + eval_real.py standalone onnxruntime evaluator. 111 real photos fetched, 61 accepted (49 train + 12 strict-val); three retrain checkpoints compared on the real split (12 images/39 GT boxes): OLD (syn) mAP50 0.860/mAP(50-95)0.662 — DECISION: keep synthetic-only as runtime (best box quality); self-training on auto-labels hurt box quality (mAP50-95 0.662→0.511), so future retraining must use higher-quality annotations first | backend/ml/training/{fetch_real_photos.py,annotate_real_photos.py,build_combined_dataset.py,eval_real.py}, backend/ml/data/real/{CREDITS.md,manifest.json,annotate_report.json,contact_sheet.jpg,images/{train,val},labels/{train,val},raw/}, ml/training/README.md | VERIFIED | Real-split eval: OLD mAP50 0.860 (best), NEW2 (noisy) 0.741, NEW3 (strict 23 imgs) 0.789; per-class: OLD package AP50 0.629/label 0.695; NEW3 package 0.492/label 0.531; recommendation: gate future retraining on manual contact-sheet review before auto-label use | 111 open-licensed CC photos in raw (not committed); credits file | AI Agent |
