...
def upload_report(data: bytes, inspection_id: str, filename: str = "report.pdf") -> str:
    """Upload a generated report artifact to object storage.

    Reports are generated by WeasyPrint per prd.md §24 and stored
    for retrieval by inspectors. Also used for JSON exports.

    Args:
        data: Report document bytes (PDF or JSON).
        inspection_id: UUID of the inspection.
        filename: Object key filename (default: report.pdf).

    Returns:
        Storage URL: minio://lm-reports/{inspection_id}/{filename}
    """
    s3 = _get_s3_client()
    key = f"{inspection_id}/{filename}"
    ...
    logger.info(f"Uploaded report: {key} ({len(data)} bytes)")
